...
const collectionConfigs: Record<string, ShopifyCollectionPageConfig> = {
  ...
  'bridal-party-outfits': {
    ...
    name: 'Bridal Party Outfits',
    eyebrow: 'Coordinated Indian Wedding Style',
    ...
    intro: 'Build a coordinated wedding party without requiring every person to wear the identical outfit. Browse lehengas, sarees, suits and menswear selected for bridesmaids, family members and wedding attendants.',
    editorialTitle: 'How to Coordinate an Indian Bridal Party',
    editorial: [
      'Start with a shared color family, fabric weight or embroidery detail, then allow silhouettes that suit each person and ceremony. Coordinating one design element usually photographs more naturally than forcing every member of the party into the same cut.',
      'Before ordering, compare event date, sizes, alteration time and availability for the full group. For larger or mixed-size parties, contact our styling team so we can check the current product details together.',
    ],
    faqs: [
      { question: 'Do bridal party outfits need to match exactly?', answer: 'No. A coordinated palette, border, fabric or embroidery detail can create a cohesive group while allowing different silhouettes and sizes.' },
      { question: 'Can you help coordinate bridesmaids and family outfits?', answer: 'Yes. Share your event date, party size, color palette, sizes and budget with our styling team for recommendations from currently available products.' },
      { question: 'Should I order every bridal party outfit at once?', answer: 'If exact matching matters, confirm current availability before ordering. Product batches and shades can change, so coordinated orders should be planned as early as possible.' },
    ],
  },
};
