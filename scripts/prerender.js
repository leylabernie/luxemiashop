...
function generateHtml(template, route, allShopifyProducts) {
  ...
  if (route.path.startsWith('/product/')) {
    ...
    // NOTE: FAQPage JSON-LD for product pages is emitted by the product-body
    // section below, built from the same `productQuestions` array as the
    // visible "Product Questions" markup (per Google's requirement that
    // structured data mirror visible content). A second, hardcoded FAQPage
    // block here would duplicate it on every PDP — do not re-add.
