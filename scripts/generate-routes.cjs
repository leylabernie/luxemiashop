 ...
 */
function generateAutoRoutesTs(routes, productHandles) {
  const routeLines = routes.map((r) => `  '${r}',`).join('\n');
  const handleLines = (productHandles || []).map((h) => `  '${h}',`).join('\n');
...
// Every product handle that has a prerendered HTML file at /_prerender/product/<handle>.html.
// Middleware uses this to rewrite product page requests without making a self-HTTP HEAD check.
 ...
 */
function writeOutputFiles(routes, productHandles) {
  ...
  // Write src/lib/autoRoutes.ts (includes product handles for middleware)
  const tsContent = generateAutoRoutesTs(routes, productHandles || []);
  fs.writeFileSync(AUTO_ROUTES_TS, tsContent, 'utf8');
  console.log(
    `[generate-routes] Written ${AUTO_ROUTES_TS} (${routes.length} routes, ${(productHandles || []).length} product handles)`
  );
...
async function main() {
  ...
  writeOutputFiles(finalRoutes, productHandles);

  const blogCount = finalRoutes.filter((r) => r.startsWith('/blog/')).length;
  console.log(
    `[generate-routes] Done: ${STATIC_ROUTES.length} static + ${blogCount} blog + ${productHandles.length} product handles in manifest = ${finalRoutes.length} total routes`
  );
}
